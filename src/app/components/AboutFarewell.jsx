import React from 'react'
import Link from 'next/link'

const AboutFarewell = () => {
  return (
    <p className=' w-5/6 md:w-11/12 lg:w-2/3 m-auto mb-4 text-center  md:text-2xl'>
    Thank you for visiting <Link href='/' className='font-medium underline decoration-red-500'>E-books-Online</Link>. 
    We hope you find inspiration, entertainment, and knowledge within our pages. If you have any questions or need assistance, feel free to {''}  
    <Link href='/contact' className='font-medium underline decoration-blue-500'>reach out to our customer support team.</Link>
  </p>
  )
}

export default AboutFarewell